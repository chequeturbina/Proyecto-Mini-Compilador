...
*/
int addSym(symtab* st, symbol* sym){
    if(st){
        int posicion = getId(st, sym->id);
        if(posicion == -1){
            st->num++;
            if(st->root == NULL){
 ...
 */
listParam* getArgs(symtab* st, char* id){
    int posicion = getId(st, id);
    if (posicion == -1){
